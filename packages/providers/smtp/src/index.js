import { InvalidConfigurationError, ProviderUnavailableError, RateLimitError, normalizeRecipients } from "@email-sdk/core";
function validateConfig(config) {
    if (!config.host || config.port <= 0 || !config.username || !config.password || !config.fromEmail) {
        throw new InvalidConfigurationError("SMTP config requires host, port, credentials, and fromEmail", "smtp");
    }
}
function mapBehavior(config) {
...
function mapRequestToSmtpPayload(request, fromEmail) {
    return {
